...
def myevents():
    ...
    for x in parts:
        parts_list[x.id] = [Participant.query.filter_by(e_id=x.id).all()]
        parts_list[x.id].append(len(parts_list[x.id][0]))
    return render_template('myevents.html', title='My Events', posts=posts,join=participant_list, parts=parts, joinp=parts_list)
...
def create():
    ...
    if form.validate_on_submit():
        ...
        event = Event(date=form.date.data, time=form.time.data, cost=form.cost.data, np=form.np.data,
                      creator=current_user, place=c, sportevent=s, level=form.level.data)
